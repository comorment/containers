...
# README:
# pgs.py converts pheno+dict files into format compatible with plink or regenie analysis. Other association analyses can be added later.
# pgs.py automatically decides whether to run logistic or linear regression by looking at the data dictionary for requested phenotypes
# pgs.py generates all scripts needed to run the analysis, and to convert
# the results back to a standard summary statistics format
...
class LoadFromFile (argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        with values as f:
            contents = '\n'.join(
                [x for x in f.readlines() if (not x.strip().startswith('#'))])
...
def parse_args(args):
    ...
    parent_parser.add_argument(
        ...
        type=str,
        default=None,
        help="file to output log, defaults to <out>.log")
    parent_parser.add_argument(
        "--log-append",
        action="store_true",
        default=False,
    ...
    pheno_parser.add_argument(
        ...
        type=str,
        default="config.yaml",
        help="file with misc configuration options")
    ...
    filter_parser.add_argument(
        ...
        type=str,
        default=None,
        help="File with SNP and INFO columns. Values in SNP column must be unique.")
    filter_parser.add_argument(
        "--info",
        type=float,
        default=None,
        help="threshold for filtering on imputation INFO score")
    filter_parser.add_argument(
        "--maf",
        type=float,
        default=None,
        help="threshold for filtering on minor allele frequency")
    filter_parser.add_argument(
        "--hwe",
        type=float,
        default=None,
    ...
    parser_pgrs_add_arguments(
        ...
        parser=subparsers.add_parser(
            ...
            parents=[
                parent_parser,
                pheno_parser],
            help='compute polygenic risk score'))

    parser_merge_plink2_add_arguments(
        args=args,
        func=merge_plink2,
        parser=subparsers.add_parser(
            "merge-plink2",
            parents=[
                parent_parser,
                filter_parser],
            help='merge plink2 sumstats files'))
    parser_merge_regenie_add_arguments(
        args=args,
        func=merge_regenie,
        parser=subparsers.add_parser(
            "merge-regenie",
            parents=[
                parent_parser,
                filter_parser],
            help='merge regenie sumstats files'))
    parser_merge_saige_add_arguments(
        args=args,
        func=merge_saige,
        parser=subparsers.add_parser(
            "merge-saige",
            parents=[
                parent_parser,
                filter_parser],
            help='merge saige sumstats files'))
...
def parser_pgrs_add_arguments(args, func, parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="plink file to use for LD structure estimation")
    parser.add_argument(
        "--sumstats",
        type=str,
        help="Input file with summary statistics")
    parser.add_argument(
        '--analysis',
        type=str,
        default=['prsice2'],
    ...
    parser.add_argument(
        ...
        default=[
            ...
            0.5,
            1.0],
        help="p-value threshold for independent significant SNPs.")
    parser.add_argument(
        "--keep-ambig",
        action="store_true",
...
def extract_variables(df, variables, pheno_dict_map, log):
    ...
    for var in cat_vars:
        ...
        log.log(
            'Variable {} will be extracted as dummie, dropping {} label (most frequent)'.format(
                var,
                drop_col))
    return dummies.copy()
...
def remove_suffix(text, suffix):
    return text[:-len(suffix)
                ] if text.endswith(suffix) and len(suffix) != 0 else text
...
def fix_and_validate_pheno_args(args, log):
    if not args.pheno_file:
        raise ValueError('--pheno-file is required.')
    if not args.pheno:
        raise ValueError('--pheno is required.')

    if len(set(args.pheno)) != len(args.pheno):
        raise ValueError(
            'check you --pheno argument, some phenotypes listed more then once')
    if len(set(args.covar)) != len(args.covar):
        raise ValueError(
            'check you --covar argument, some covariates listed more then once')
    ...
    if args.dict_sep is None:
        args.dict_sep = args.pheno_sep
    if args.dict_sep in sep_map:
        args.dict_sep = sep_map[args.dict_sep]
    ...
    if not args.dict_file:
        args.dict_file = args.pheno_file + '.dict'
...
def fix_and_validate_gwas_args(args, log):
    if len(set(['plink2', 'regenie', 'saige']
               ).intersection(set(args.analysis))) > 1:
        raise ValueError(
            '--analysis can have only one of plink2, regenie, saige; please choose one of these.')

    if (args.info is not None) or (args.info_file is not None):
        if (args.info is None) or (args.info_file is None):
            raise ValueError(
                'both --info and --info-file must be used at the same time')
        check_input_file(args.info_file, chr2use=args.chr2use)

    if ('regenie' in args.analysis) and (not args.geno_fit_file):
        raise ValueError(
            '--geno-fit-file must be specified for --analysis regenie')

    if ('saige' in args.analysis) and (not args.geno_fit_file):
        raise ValueError(
            '--geno-fit-file must be specified for --analysis saige')

    if ('saige' in args.analysis) and (
            args.chunk_size_bp is not None) and (not args.bim):
        raise ValueError(
            '--bim must be specified together with --chunk-size-bp in case of --analysis saige')
...
def pass_arguments_along(args, args_list):
    ...
    return ''.join([(' --{} {} '.format(arg, '' if (val) else val)
                   if val else '') for arg, val in zip(args_list, vals)])
...
def prepare_covar_and_phenofiles(args, log, cc12, join_covar_into_pheno):
    ...
    pheno, pheno_dict = read_comorment_pheno(
        args, args.pheno_file, args.dict_file)
    pheno_dict_map = dict(zip(pheno_dict['FIELD'], pheno_dict['TYPE']))

    missing_cols = [str(c) for c in args.pheno if (c not in pheno.columns)]
    if missing_cols:
        raise (ValueError(
            '--pheno not present in --pheno-file: {}'.format(', '.join(missing_cols))))
    ...
    for pheno_type in pheno_types:
        if pheno_type not in ['BINARY', 'CONTINUOUS']:
            raise (
                ValueError('only BINARY or CONTINOUS varibales can be used as --pheno'))
    ...
    num_jobs = len(pheno)
    pheno = pd.merge(pheno, fam[['IID', 'FID']], on='IID', how='inner')
    pheno_dict_map['FID'] = 'FID'
    log.log(
        "n={} individuals remain after merging, n={} removed".format(
            len(pheno),
            num_jobs -
            len(pheno)))
    ...
    if args.covar:
        ...
        if missing_cols:
            raise (ValueError(
                '--covar not present in --pheno-file: {}'.format(', '.join(missing_cols))))
        ...
        for var in args.covar:
            ...
            if np.any(mask):
                ...
                log.log(
                    "n={} individuals remain after removing n={} individuals with missing value in {} covariate".format(
                        len(pheno),
                        num_jobs -
                        len(pheno),
                        var))
    ...
    if args.variance_standardize is not None:
        if len(args.variance_standardize) == 0:
            args.variance_standardize = [
                col for col in pheno.columns if (
                    pheno_dict_map[col] == 'CONTINUOUS')]

        for col in args.variance_standardize:
            if (col not in pheno.columns) or (
                    pheno_dict_map[col] != 'CONTINUOUS'):
                raise ValueError(
                    'Can not apply --variance-standardize to {}, column is missing or its type is other than CONTINUOUS'.fromat(col))

            mean = np.nanmean(pheno[col].values)
            std = np.nanstd(pheno[col].values, ddof=1)
            log.log(
                'phenotype {} has mean {:.5f} and std {:.5f}. Normalizing to 0.0 mean and 1.0 std'.format(
                    col,
                    mean,
                    std))
    ...
    if not join_covar_into_pheno:
        ...
        if args.covar:
            covar_output = extract_variables(
                pheno, args.covar, pheno_dict_map, log)
            log.log(
                'writing {} columns (including FID, IID) for n={} individuals to {}.covar'.format(
                    covar_output.shape[1],
                    len(covar_output),
                    args.out))
    ...
    log.log("extracting phenotypes{}...".format(
        ' and covariates' if join_covar_into_pheno else ''))
    pheno_and_covar_cols = args.pheno + \
        (args.covar if join_covar_into_pheno else [])
    pheno_output = extract_variables(
        pheno, pheno_and_covar_cols, pheno_dict_map, log)
    for var in pheno_and_covar_cols:
        if pheno_dict_map[var] == 'BINARY':
            log.log(
                'variable: {} ({}), cases: {}, controls: {}, missing: {}'.format(
                    var, pheno_dict_map[var], np.sum(
                        pheno[var] == '1'), np.sum(
                        pheno[var] == '0'), np.sum(
                        pheno[var].isnull())))
        elif pheno_dict_map[var] in ['NOMINAL', 'ORDINAL']:
            counts = '; '.join(
                ["'{}' - {}".format(val, np.sum(pheno[var] == val)) for val in pheno[var].unique()])
            log.log(
                'variable: {} ({}), value counts: {}'.format(
                    var, pheno_dict_map[var], counts))
        else:
            log.log(
                'variable: {} ({}), missing: {}'.format(
                    var, pheno_dict_map[var], np.sum(
                        pheno[var].isnull())))
    ...
    if cc12 and (pheno_type == 'BINARY'):
        ...
        for var in args.pheno:
            pheno_output[var] = pheno_output[var].map(
                {'0': '1', '1': '2'}).values

    log.log(
        'writing {} columns (including FID, IID) for n={} individuals to {}.pheno'.format(
            pheno_output.shape[1],
            len(pheno_output),
            args.out))
    pheno_output.to_csv(
        args.out +
        ...
        index=False,
        sep='\t')
    fam[['IID']].to_csv(args.out + '.sample', na_rep='NA',
                        header=None, index=False, sep='\t')

    if join_covar_into_pheno:
        # update args.covar so that it reflects dummies
        args.covar = [col for col in pheno_output.columns if (
            col not in (['IID', 'FID'] + args.pheno))]
...
def execute_pgrs(args, log):
    ...
    if not args.geno_file:
        raise ValueError('--geno-file is required.')
    if (not is_bed_file(args.geno_file)) and (
            not is_bgen_file(args.geno_file)):
        raise ValueError('--geno-file must be .bed or .bgen')

    if not args.geno_ld_file:
        raise ValueError('--geno-ld-file is required.')
    if (not is_bed_file(args.geno_ld_file)) and (
            not is_bgen_file(args.geno_ld_file)):
        raise ValueError('--geno-ld-file must be .bed or .bgen')

    pheno_type = prepare_covar_and_phenofiles(
        args, log, cc12=True, join_covar_into_pheno=False)
...
def apply_filters(args, df):
    ...
    if (args.info is not None) or (args.info_file is not None):
        ...
        info = pd.concat([pd.read_csv(args.info_file.replace('@',
                                                             chri),
                                      delim_whitespace=True,
                                      dtype={'INFO': np.float32})[['SNP',
                                                                   'INFO']] for chri in chr2use])
        if np.any(info['SNP'].duplicated()):
            raise (
                ValueError("SNP column has duplicated values in --info-file file"))
        ...
        log.log(
            "n={} SNPs remain after merging, n={} with missing INFO score".format(
                len(df), np.sum(
                    df['INFO'].isnull())))

        n = len(df)
        df = df[~(df['INFO'] < args.info)].copy()
        log.log(
            "n={} SNPs remain after filtering INFO>={}, n={} removed".format(
                len(df),
                args.info,
                n -
                len(df)))
    ...
    if args.maf is not None:
        ...
        if np.any(maf['ID'].duplicated()):
            raise (
                ValueError(
                    "ID column has duplicated values in {}.afreq file - have your .bim file had duplicated SNPs?".format(
                        args.basename)))
        maf.rename(columns={'ID': 'SNP'}, inplace=True)
        df = pd.merge(df, maf, how='left', on='SNP')
        n = len(df)
        df = df[(df['ALT_FREQS'] >= args.maf) & (
            df['ALT_FREQS'] <= (1 - args.maf))].copy()
        log.log(
            "n={} SNPs remain after filtering allele frequency on {}<=FRQ<={}, n={} removed".format(
                len(df),
                args.maf,
                1 -
                args.maf,
    ...
    if args.hwe is not None:
        hwe = pd.concat([pd.read_csv(args.basename.replace(
            '@', chri) + '.hardy', delim_whitespace=True)[['ID', 'P']] for chri in args.chr2use])
        if np.any(hwe['ID'].duplicated()):
            raise (
                ValueError(
                    "ID column has duplicated values in {}.hardy file - have your .bim file had duplicated SNPs?".format(
                        args.basename)))
        hwe.rename(columns={'ID': 'SNP', 'P': 'P_HWE'}, inplace=True)
        df = pd.merge(df, hwe, how='left', on='SNP')
        n = len(df)
        df = df[df['P_HWE'] >= args.hwe].copy()
        log.log(
            "n={} SNPs remain after filtering Hardy Weinberg equilibrium P>={}, n={} removed".format(
                len(df),
                args.hwe,
                n -
                len(df)))
    ...
    if args.geno is not None:
        vmiss = pd.concat([pd.read_csv(args.basename.replace(
            '@', chri) + '.vmiss', delim_whitespace=True)[['ID', 'F_MISS']] for chri in args.chr2use])
        if np.any(vmiss['ID'].duplicated()):
            raise (
                ValueError(
                    "ID column has duplicated values in {}.vmiss file - have your .bim file had duplicated SNPs?".format(
                        args.basename)))
        vmiss.rename(columns={'ID': 'SNP'}, inplace=True)
        df = pd.merge(df, vmiss, how='left', on='SNP')
        n = len(df)
        df = df[df['F_MISS'] <= args.geno].copy()
        log.log(
            "n={} SNPs remain after filtering SNPs missingness F_MISS>={}, n={} removed".format(
                len(df),
                args.geno,
                n -
                len(df)))
...
def merge_plink2(args, log):
    ...
    effect_cols = (['BETA', "SE"] if pattern.endswith(
        '.glm.linear') else ['OR', 'LOG(OR)_SE'])
    ct_cols = ([] if pattern.endswith('.glm.linear')
               else ["CASE_ALLELE_CT", "CTRL_ALLELE_CT"])
    check_input_file(pattern, args.chr2use)
    df = pd.concat([pd.read_csv(pattern.replace('@',
                                                chri),
                                delim_whitespace=True)[['ID',
                                                        '#CHROM',
                                                        'POS',
                                                        'REF',
                                                        'ALT',
                                                        ...
                                                        'P',
                                                        'L95',
                                                        'U95'] + ct_cols + effect_cols] for chri in args.chr2use])
    df['A2'] = df['REF']
    idx = df['A2'] == df['A1']
...
def merge_regenie(args, log):
    ...
    df = pd.concat([pd.read_csv(pattern.replace('@',
                                ...
                                delim_whitespace=True)[['ID',
                                                        ...
                                                        'A1FREQ',
                                                        'N',
                                                        'LOG10P']] for chri in args.chr2use])
    df['P'] = np.power(10, -df['LOG10P'])
    df['Z'] = -stats.norm.ppf(df['P'].values * 0.5) * \
        np.sign(df['BETA']).astype(np.float64)
    df.dropna(inplace=True)
    df.rename(
        columns={
            'ID': 'SNP',
...
def merge_saige(args, log):
    ...
    # binary GWAS columns:       CHR POS SNPID Allele1 Allele2 AC_Allele2
    # AF_Allele2 imputationInfo N BETA SE Tstat p.value p.value.NA
    # Is.SPA.converge varT varTstar AF.Cases AF.Controls N.Cases N.Controls

    fix_and_validate_chr2use(args, log)
    pattern = args.sumstats
    chr_or_chunk_labels = list(
        range(
            1,
            args.chunks +
            1)) if args.chunks else args.chr2use
    check_input_file(pattern, chr_or_chunk_labels)
    df = pd.concat([pd.read_csv(pattern.replace('@', str(label)),
                   delim_whitespace=True) for label in chr_or_chunk_labels])
    ...
    df, info_col = apply_filters(args, df)
    df[['SNP', 'CHR', 'BP', 'A1', 'A2', 'N'] +
       (['CaseN', 'ControlN'] if logistic else []) +
        info_col +
        ['FRQ', 'Z', 'BETA', 'SE', 'P']].to_csv(args.out, index=False, sep='\t')
    os.system('gzip -f ' + args.out)
    write_readme_file(args)
    ...
    if (chr2use is not None) and ('@' in fname):
        for chri in chr2use:
            if not os.path.isfile(fname.replace('@', str(chri))):
                raise ValueError(
                    "Input file does not exist: {f}".format(
                        f=fname.replace(
                            '@', str(chri))))
    else:
...
def read_bim(args, bim_file):
    ...
    bim = pd.read_csv(
        ...
        delim_whitespace=True,
        header=None,
        names='CHR SNP GP BP A1 A2'.split())
...
def read_iid_from_keep_or_remove_file(args, fname):
    ...
    log.log(
        'done, {} rows, {} cols, {} unique IIDs taken from {} column'.format(
            len(df),
            df.shape[1],
            len(iid),
            col_idx +
...
def read_fam(args, fam_file):
    ...
    fam = pd.read_csv(
        ...
        delim_whitespace=True,
        header=None,
        names='FID IID FatherID MotherID SEX PHENO'.split(),
        dtype=str)
    ...
    with open(cmd_file, 'a') as f:
        for command in commands:
            if array_spec is not None:
                f.write(
                    'for SLURM_ARRAY_TASK_ID in {}; do {}; done\n'.format(
                        ' '.join(array_spec), command))
            else:
                f.write('{}\n'.format(command))

    if (slurm_job_index != 1):
        submit_jobs_list.append(
            'RES=$(sbatch --dependency=afterok:${{RES##* }} {})'.format(slurm_job_file))
    else:
...
def read_comorment_pheno(args, pheno_file, dict_file):
    ...
    extra_values = list(set(pheno_dict['TYPE'].values).difference(
        {'IID', 'CONTINUOUS', 'BINARY', 'NOMINAL'}))
    if len(extra_values) > 0:
        raise (
            ValueError(
                'TYPE column in --dict can only have IID, CONTINUOUS, BINARY and NOMINAL - found other values, e.g. {}'.format(
                    extra_values[0])))

    # validation logic for pheno file
    if 'IID' not in pheno:
        raise (ValueError('IID column not present in --pheno-file'))
    if np.any(pheno['IID'].duplicated()):
        raise (ValueError('IID column has duplicated values in --pheno-file'))
    missing_cols = [
        str(c) for c in pheno.columns if (
            c not in pheno_dict['FIELD'].values)]
    if missing_cols:
        raise (ValueError(
            '--pheno-file columns not present in --dict: {}'.format(', '.join(missing_cols))))
    ...
    if len(keep) > 0:
        ...
        log.log(
            '{} individuals remain after applying --keep file(s)'.format(len(pheno)))
        if len(pheno) == 0:
            raise ValueError(
                'no individuals left after applying --keep file(s)')

    if len(remove) > 0:
        pheno = pheno[~pheno['IID'].isin(remove)]
        log.log(
            '{} individuals remain after applying --remove file(s)'.format(len(pheno)))
        if len(pheno) == 0:
            raise ValueError(
                'no individuals left after applying --keep file(s)')
...
if __name__ == "__main__":
    ...
    try:
        ...
        log.log('Beginning analysis at {T} by {U}, host {H}'.format(
            T=time.ctime(), U=getpass.getuser(), H=socket.gethostname()))
