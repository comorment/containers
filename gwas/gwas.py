...
def make_manh(args, pheno, analysis):
...
def make_regenie_merge(args, logistic):
    ...
    for pheno in args.pheno:
        cmd += '$PYTHON gwas.py merge-regenie ' + \
            ...
            ' --chr2use {} '.format(','.join(args.chr2use)) + \
            '\n'
        if args.loci: cmd += make_loci(args, pheno, 'regenie')
        if args.manh: cmd += make_manh(args, pheno, 'regenie')
...
def make_plink2_merge(args, logistic):
    ...
    for pheno in args.pheno:
        cmd += '$PYTHON gwas.py merge-plink2 ' + \
            ...
            ' --chr2use {} '.format(','.join(args.chr2use)) + \
            '\n'
        if args.loci: cmd += make_loci(args, pheno, 'plink2')
        if args.manh: cmd += make_manh(args, pheno, 'plink2')
    return cmd
