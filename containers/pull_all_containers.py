...
# Defining things manually as some registries may have many tags and we don't want them all.
# We may also choose to sync only registry pulls without git repos/codes

# parse input tags
parser = argparse.ArgumentParser(description='Pull all containers from registry')
    ...
    for tag in tags:
        ...
        if not os.path.isfile(fname):
            try:
                ...
                os.system(f'oras pull -o {tag} {ref}_sif:{tag}')  # note "_sif" suffix
            except AssertionError:
                try:
                    shutil.which('apptainer') is not None
                except AssertionError:
                    mssg = 'Neither oras nor apptainer executables found. Please install one of them.'
                    raise Exception(mssg)
                print(f'creating build {fname}:')
                with tempfile.TemporaryDirectory() as tmpdir:
                    os.system(f'export APPTAINER_CACHEDIR={tmpdir} && apptainer pull {fname} docker://{ref}:{tag}')
    
    # if 'latest' not in tags, symlink tag with highest semantic version:
    cwd = os.getcwd()
    if not 'latest' in tags:
        try:
        ...
        if create_tag_dir:
            ...
            os.system(f"ln -s -f {f'{prefix}_{tag}.sif'} {f'{prefix}_latest.sif'}")
        os.chdir(cwd)
