...
Now, to run this use casse, just copy the [gwas.py](../gwas/gwas.py) script from ``<COMORMENT/containers/gwas/gwas.py>>`` into your current folder, and run the following commands (where ``run1`` gives example of case/control GWAS, while ``run2`` is an example for quantitative traits):
```
singularity exec --home $PWD:/home $SIF/python3.sif python gwas.py gwas \
--argsfile /REF/examples/regenie/example_3chr.argsfile --pheno CASE CASE2 --covar PC1 PC2 BATCH --out run1

singularity exec --home $PWD:/home $SIF/python3.sif python gwas.py gwas \
--argsfile /REF/examples/regenie/example_3chr.argsfile --pheno PHENO PHENO2 --covar PC1 PC2 BATCH --out run2
```
...
```
run1_CASE.plink2.gz
run1_CASE.regenie.gz
run1_CASE2.plink2.gz
run1_CASE2.regenie.gz
run2_PHENO.plink2.gz
run2_PHENO.regenie.gz
run2_PHENO2.plink2.gz
